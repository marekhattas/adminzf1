...
{   
    ...
    public function __construct() {
        $this->config = new Zend_Config_Ini(APPLICATION_PATH.'/configs/plugins.ini',APPLICATION_ENV);
    }
    ...
    public function convertVideo($filePath, $fileName){
        ...
        if(file_exists($filePath.$fileName)){
            $newFilename = str_replace('.', '-', $fileName).'.mp4';
            //shell_exec('('.$this->getConfig()->system->ffmpeg->path.' -y -i '.$filename.' -vcodec h264 -acodec aac -strict -2 '.$newFilename.' && rm '.$filename.') > /dev/null 2>&1 &');
            shell_exec('('.$this->getConfig()->system->ffmpeg->path.' -y -i '.$filePath.$fileName.' -vcodec h264 -acodec aac -strict -2 '.$filePath.$newFilename.' && rm '.$filePath.$fileName.') > '.APPLICATION_PATH.'/../ffmpeg.log 2>&1 &');
            return $newFilename;
        }
        return false;
    }
    ...
    /*
     * Create snapshot of video in JPG format
     * @param string $filename full filename with path
     * @return boolean
     */
    public function createThumbnail($filePath, $fileName) {
        ...
        if(file_exists($filePath.$fileName)){
            $newFilename = str_replace('.', '-', $fileName).'.jpg';
            shell_exec($this->getConfig()->system->ffmpeg->path.' -y -i '.$filePath.$fileName.' -ss '.$this->getConfig()->system->ffmpeg->thumbnailTime.' -vframes 1 '.$filePath.$newFilename.' > '.APPLICATION_PATH.'/../ffmpeg.log 2>&1');
            return $newFilename;
        }
        return false;
    }
